...
export const trackOtpRequests = async (email: string, next: NextFunction) => {
  ...
  let otpRequests = parseInt((await redis.get(optRequestKey)) || "0");
  if (otpRequests >= 2) {
  ...
  await redis.set(optRequestKey, otpRequests + 1, "EX", 3600);// this is setting the count, default is 0
};
...
export const sendOtp = async (
  ...
  await redis.set(`otp:${email}`, otp, "EX", 300); // five minutes expiary
  await redis.set(`otp_cooldown:${email}`, "true", "EX", 60); //one minute for another request
  //
};
